...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $item = Category::with(['experts' => function ($query) {
                    $query->select('experts.id', 'name')  
                          ->withCount([
                              'leads as confirmed_leads_count' => function ($query) {
                                  $query->where('is_confirm', 1); 
                              },
                              'leads as pending_leads_count' => function ($query) {
                                  $query->where('is_confirm', 0);
                              }
                          ])
                        ->withSum('recharges as recharge_amount', 'amount', function ($query) {
                              $query->where('type', 'Credit');
                          })
                        ->withSum('recharges as recharge_points', 'points', function ($query) {
    ...
    // return response()->json($item);
        ...
        foreach($item as $itm){
            $pt = 0;
            $am = 0;
            foreach($itm['experts'] as $exp){
                $pt += $exp['balance']['total_debit'];
                $am += $exp['recharge_amount'];
            }
            $iobj = [
                'category' => $itm['category'],
                'experts' => count($itm['experts']),
                'points' => $pt,
                'amount' => $am
                ];
            array_push($data, $iobj);
        }
    ...
    public function users(){
        ...
        $res['items'] = Lead::whereHas('search_data')->with(['search_data'])->orderBy('id', 'DESC')->paginate(30);
    ...
    public function add_points(Request $request){
        ...
        return redirect()->back()->with('success', 'points credited successfully');
    }
    
   public function transactions(){
    $expert = $_GET['expert'] ?? null;
    $fdate = $_GET['fdate'] ?? null;
    $tdate = $_GET['tdate'] ?? null; // Corrected 'edate' to 'tdate'
    $rows = $_GET['rows'] ?? 50;
    $type = $_GET['type'] ?? null;
    ...
    $res['expert'] = $expert;
    // Start the query
    $items = ExpertPoint::orderBy('id', 'DESC');
   

    // Apply filters
    if ($expert) {
        
        $items->whereIn('expert_id', function($query)use($expert){
            $query->from('experts')->select('id')->where('name', 'LIKE', "%{$expert}%")->orWhere('email', 'LIKE', "%{$expert}%")->orWhere('mobile', 'LIKE', "%{$expert}%");
        });
    }
    // if ($fdate && $tdate) {
    //     $items->whereBetween('created_at', [$fdate, $tdate]);
    // }
    if ($fdate) {
        $items->whereDate('created_at', $fdate);
    }
    ...
    if($amount){
        $items->where('amount', $amount);
    }
    if($type){
        $items->where('type', $type);
    }
    if($status){
        $items->where('is_confirm', $status == "Success" ? "1" : "0");
    }
    ...
    $rows = $items->with('expert')->paginate(2000);

    // Prepare the response
    $res['items'] = $rows;
    $res['title'] = "List of transactions";
    if (isset($_GET['export']) && $_GET['export'] == 'excel') {
        $filters = [];
        return Excel::download(new TransactionExport($filters), 'transactions.xlsx');
    }
    // Check the response in JSON
    // return response()->json($res['items'])

    // If you want to return a view (remove `die` statement)
