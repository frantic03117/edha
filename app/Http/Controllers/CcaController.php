...
class CcaController extends Controller
{
    ...
    function ccAvenueResponse()
    {
        ...
        $ostatus = $status == "Success" ? "1" : "0";
        $orderdetails = ExpertPoint::where('order_id', $data[0]['order_id'])->first();
        $oamount = $orderdetails['amount'];
        $eid = $orderdetails['expert_id'];
        $expert = Expert::where('id', $eid)->first();
        $ename = $expert['name'];
        $emobile =  $expert['mobile'];
        ExpertPoint::where('order_id', $data[0]['order_id'])->update(['cca_response' => json_encode($data), 'is_confirm' => $ostatus,  'payment_status' => $status]);
        if($status == "Success"){
        $msg = "{$ename} has recharged their Account with INR {$oamount} to reach out to their Clients. Team Edha";
        $resp = $this->send_sms('918920880011', $msg);
        $ms2 = "Hi {$ename}, Recharge successful. We have received your payment of INR {$oamount}. You may now get in touch with your Clients. Thanks, Edha";
        $this->send_sms('91'.$emobile, $ms2);
        $body1 = "<p> Dear $ename,</p>";
        $body1 .= "<p>Your recharge of Rs $oamount  is successful.</p>";
        $body1 .= "<p>Please feel free to get in touch with your Clients.</p>";
        $body1 .= "<p>Best regards,</p>";
        $body1 .= "<p>Team Edha</p>";
        $mailData1 = [
            'subject' => 'Account recharge successful',
            'body' => $body1
            ];
        Mail::to($expert['email'])->send(new CustomMail($mailData1));
        $body = "<p> Dear Team,</p>";
        $body .= "<p>$ename recharged their account for Rs $oamount is successful.</p>";
        $body .= "<p>Best regards,</p>";
        $body .= "<p>Team Edha</p>";
        $mailData = [
            'subject' => 'Account recharge successful : '.$ename,
            'body' => $body
            ];
        Mail::to('admin@edha.life')->send(new CustomMail($mailData));
        return redirect()->to('/');
        }else{
            return redirect()->to('/');
        }
